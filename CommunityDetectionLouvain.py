...
g = nx.read_edgelist("../data/trust_data.edgelist",create_using=nx.Graph(), nodetype = int)
partition = community_louvain.best_partition(g)

grouped_nodes_v3 = {community: [node for node,comm in partition.items() if comm == community] for community in set(partition.values())}
print(len(grouped_nodes_v3))

# Function to find key by value 
def get_key(val): 
    for key, value in grouped_nodes_v3.items(): 
        for l in value:
            if val == l: 
                return key

def getCommu(value_to_find):
    result = get_key(value_to_find)
    if result is not None:
        commu=list(grouped_nodes_v3[result])
        return commu
    else : 
        return []
    ...
    def train_model(self, k):
        ...
        while iteration < self.config.maxIter:
            ...
            for index, line in enumerate(self.rg.trainSet()):
                ...
                social_term_p, social_term_loss = np.zeros((self.config.factor)), float(0.0)
                followees = getCommu(user)
                ...
                for follower in followees:
                        if self.rg.containsUser(follower):
                            ...
                            social_term_m = float(social_term_m)
